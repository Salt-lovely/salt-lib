...
function innerParse<T>(str: string | null): T | null {
  if (str) {
    try {
...
export function unsafeRead<T>(key: string): T | null {
...
export function readAndListen<T>(props: {
  ...
  const fn = (ev: StorageEvent) => {
    ...
    const newValue = innerParse<T>(ev.newValue)
    const oldValue = innerParse<T>(ev.oldValue)
