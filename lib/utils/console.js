...
export function $warn(...args) {
    ...
    return new Promise((res) => res(args[args.length - 1])).then(console.warn.bind(console, ...args.slice(0, args.length - 1)));
}
/** 在控制台打印**报错**内容，同时不显示行号（chrome、火狐测试通过） */
export function $error(...args) {
    if (!args.length)
        return new Promise((res) => res('')).then(console.error.bind(console));
    return new Promise((res) => res(args[args.length - 1])).then(console.error.bind(console, ...args.slice(0, args.length - 1)));
}
/** 在控制台打印消息内容，同时不显示行号（chrome、火狐测试通过） */
export function $info(...args) {
    if (!args.length)
        return new Promise((res) => res('')).then(console.info.bind(console));
    return new Promise((res) => res(args[args.length - 1])).then(console.info.bind(console, ...args.slice(0, args.length - 1)));
}
/** 在控制台打印Debug内容，同时不显示行号（chrome、火狐测试通过） */
export function $debug(...args) {
    if (!args.length)
        return new Promise((res) => res('')).then(console.debug.bind(console));
    return new Promise((res) => res(args[args.length - 1])).then(console.debug.bind(console, ...args.slice(0, args.length - 1)));
}
